...
public class Bot {
    ...
    private void process(Update update) {
        ...
        if (message != null && message.viaBot() != null && message.viaBot().username().equals("name_bot")) { //your bot name!
            ...
            InlineQueryResultArticle papper = buildInlineButton("papper", "\uD83E\uDD0F Бумага", "2");

            request = new AnswerInlineQuery(inlineQuery.id(), rock, scissors, papper).cacheTime(1);
            ...
            if (senderChose.equals(opponentChose)) {
                request = new EditMessageText(
                        chatId, messageId,
                        String.format(
                                "%s и %s выбрали %s. Уровень примерно ровен",
                                senderName,
                                opponentName,
    ...
    private InlineQueryResultArticle buildInlineButton(String id, String title, String callbackData) {
        return new InlineQueryResultArticle(id, title, "Готов играть!")
                .replyMarkup(
                        new InlineKeyboardMarkup(
